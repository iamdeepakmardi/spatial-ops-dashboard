    ...
    const interval = setInterval(() => {
      setServers((currentServers) =>
        currentServers.map((server) => {
          // Fluctuate load by -5% to +5%
          const change = (Math.random() - 0.5) * 10;
          const newLoad = Math.max(0, Math.min(100, server.load + change));
      ...
      <div className="absolute inset-0 z-10 pointer-events-none flex flex-col justify-between p-6 md:p-10">
        ...
        <div className="absolute top-10 right-10 w-64 pointer-events-auto">
          <div className="bg-cyber-panel backdrop-blur-md border-l-2 border-cyan-500 p-4 rounded-r-lg shadow-lg">
            <h2 className="text-xs uppercase text-gray-400 mb-4 border-b border-gray-700 pb-2">
              Cluster Status
            </h2>
            ...
            <div className="space-y-3">
              ...
              <div className="h-16 flex items-end gap-1 mb-2 border-b border-gray-800 pb-2">
                ...
                {[...Array(10)].map((_, i) => (
                  <div
                    key={i}
                    className="flex-1 bg-cyan-900/50 hover:bg-cyan-400 transition-colors"
        ...
        <footer className="pointer-events-auto flex justify-between items-end">
          <div className="text-xs text-gray-500 max-w-md">
            <p className="mb-2">
              <span className="text-cyan-500 font-bold">[LMB]</span> Select Node
              & Focus
            </p>
            <p className="mb-2">
              <span className="text-cyan-500 font-bold">[DRAG]</span> Orbit
              Camera
            </p>
            <p>
              <span className="text-cyan-500 font-bold">[SCROLL]</span> Zoom
            </p>
          </div>
